...
class RawEntry:
    ...
    def get_processed_entry(
        ...
        if self.action == SchwabAction.CASH_DIVIDEND:
            return CashDividend(
                symbol=self.symbol,
...
class Sell(TransactionEntry):
    ...
    def get_postings(self) -> List[Posting]:
        postings = [
            ...
            Posting(
                account=f"{self.capital_gains_account}:{self.symbol}",
                units=MISSING,
                cost=None,
                price=None,
                flag=None,
...
class SchwabSource(DescriptionBasedSource):
    ...
    def prepare(self, journal: JournalEditor, results: SourceResults) -> None:
        ...
        base_accounts = tuple(processor.account_to_schwab.keys())
        account_set = {a for a in journal.accounts if a.startswith(base_accounts)}
        ...
        self._get_pending_and_invalid_entries(
            source_entries=processor.process_entries(self.raw_entries),
